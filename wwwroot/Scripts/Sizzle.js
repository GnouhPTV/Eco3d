...
function MenuTableContent() {
    // tao id cho h2,h3
    $('h2, h3, h4', $('.post-body')).each(function () {
        var id = $(this).attr('id');
    ...
    for (var i = 0; i < headings.length; i++) {
        if (headings[i].innerHTML != null) {
            ...
            var content = headings[i].innerHTML.replace(/<[^>]*>?/gm, '');
            //var top = headings[i].offsetTop;//attributes["top"].nodeValue;

            console.log(content);
            if (content != "") {
                var sId = '#' + sub_url(content);
                var sOff = $(sId).offset();
                if (typeof sOff != 'undefined') {
                    var top = $('#' + sub_url(content)).offset().top - 50;
                    var contentView = content.replace(/\d\.&nbsp;/gi, "").replace(/\d\.\s/gi, "").replace(/\d\./gi, "");
